...
> **논문의 최종 제안 구조나 주 결과가 아닙니다.** 이 결과는 요청 경로를 변경하는 초기 구현에서
> 얻었으므로 로그 기반 Taint 기법의 정확도·지연·비용 근거로 사용하지 않습니다.

> 후속 저비용 가설 설계와 1,000개 임시 지뢰·31개 Broker invocation 실측은
> [저비용 Taint 설계](LOW_COST_TAINT_DESIGN.md)와
> [비용 실험 결과](COST_STUDY_RESULTS.md)를 기준으로 한다.
